...
function perguntasSelecionadas (questoes){
    const questoesEmbaralhadas = questoes.sort(() => Math.random() - Math.random());
    const questoesSelecionadas = questoesEmbaralhadas.slice(0, 10);
    return questoesSelecionadas;
};

//Exibir as perguntas selecionadas e perguntando
function exibirSelecionadas (questoesSelecionadas){
    questoesSelecionadas.forEach((pergunta, index) => {
    console.log(`${index + 1}: ${pergunta.pergunta}`);
    let dado = entradaDados.question("Sua resposta: ");
    if(dado === pergunta.resposta){
        acertou++
...
function calculo (exibirSelecionadas){
    if(acertou == 10){
    ...
    console.log(`Pontuação final: ${acertou} acertos.`);
}

console.log("----- QUIZ DE FATOS HISTÓRICOS -----\n");
let jogador = entradaDados.question("Digite o seu nome: ")
  
calculo(exibirSelecionadas(perguntasSelecionadas(questoes)));
